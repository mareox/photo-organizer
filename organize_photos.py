...
def get_user_input():
    ...
    # Get options
    while True:
        copy_option = input("Choose operation:\n1) Copy files\n2) Move files\nEnter option (1/2): ").strip()
        if copy_option == '1':
            copy_option = True
            break
        elif copy_option == '2':
    ...
    while True:
        dry_run_option = input("Choose mode:\n1) Simulate (dry run)\n2) Actual operation\nEnter option (1/2): ").strip()
        if dry_run_option == '1':
            dry_run_option = True
            break
        elif dry_run_option == '2':
    ...
    while True:
        progress_option = input("Show progress bar?\n1) Yes\n2) No\nEnter option (1/2): ").strip()
    ...
    default_report = os.path.join(downloads_path, f"{timestamp}-organize-photos-report.csv")
    report_file = input(f"Enter path for the report file (leave empty for default '{default_report}'): ").strip()
    if not report_file:
...
def create_report_file(report_file_path, file_data, dry_run=False):
    ...
    try:
        with open(report_file_path, 'w', newline='', encoding='utf-8') as csv_file:
            ...
            for data in tqdm.tqdm(file_data, desc="Generating report"):
...
def main():
    ...
    if args.progress:
        ...
        media_files_iter = tqdm.tqdm(media_files, desc="Processing", unit="file")
    else:
